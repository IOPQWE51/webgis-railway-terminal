...
const DarkTacticalHUD = ({ stationData, onClose }) => {
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      setIsMobile(mobile);
      // 🔄 切换到移动端时重置标志，允许重新发射
      if (mobile && !hasDispatchedMobileRef.current) {
      ...
      const mobileContent = `
        <div style="padding: 20px; font-family: 'Courier New', monospace;">
          ...
          <div style="background: rgba(0, 0, 0, 0.3); border: 1px dashed rgba(251, 191, 36, 0.2); border-radius: 8px; padding: 12px; text-align: center; margin-top: 16px;">
            ...
            ${info?.apiProvider ? `<div style="color: #475569; font-size: 10px; margin-top: 6px;">预计使用: ${info.apiProvider}</div>` : ''}
          </div>
        </div>
      `;
    ...
    <>
      ...
      <div
        style={{
          ...
          height: 'calc(100vh - max(90px, 8vh) - max(50px, 6vh))',
          zIndex: 5000,
